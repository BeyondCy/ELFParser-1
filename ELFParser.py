...
class ELF32(object):
    ...
    def ReadStr(self,offset,end='\0',len=32):
        ...
        chars = ""
        for char in self.data.read(len):
            if not char == end:
                chars = chars + char
            else:
                break
...
class ELF(object):
    ...
    def __init__(self,filepath):
        with open(filepath, 'rb') as fr:
            self.data = cStringIO.StringIO(fr.read())
    ...
    def PrintELFShdr(self):
        ...
        for i in range(0,self.header.e_shnum):
            ...
            sh_flags = ''.join([mark for flag,mark in ELFSHFLAG.iteritems() if shdr.sh_flags & flag])
    ...
    def PrintELFPhdr(self):
        ...
        for i in range(0,self.header.e_phnum):
            phdr = self.segment_headers[i]
            ph_flags = ''.join([mark for flag,mark in ELFPHFLAG.iteritems() if phdr.p_flags & flag])
